...
TEMPLATE_FILE = os.path.join(BASE_DIR, "802-quick-quote-template.html")
def find_ref(substr):
    """Find a file in REF_DIR whose name contains `substr` (case-insensitive)."""
    for fname in os.listdir(REF_DIR):
        if substr.lower() in fname.lower():
            return os.path.join(REF_DIR, fname)
    raise FileNotFoundError(f"No file containing '{substr}' found in {REF_DIR}")

SPEC_CSV    = find_ref("AI FULL SPEC SHEET")
MODS_CSV    = find_ref("Cabinet Modifications")
INSERTS_CSV = find_ref("Cabinet Insert Accessories")
...
CAB_PATTERN = re.compile(
    r"^.*?(Belmont|Seaport)\s+(.+?)\s+(RTA)\.csv$", re.IGNORECASE
)
...
# ── Step 7: Write output ──────────────────────────────────────────────────────
out_path = os.path.join(BASE_DIR, "802-quick-quote.html")
